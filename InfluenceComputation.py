...
#   Parameters
countries = ['A', 'B', 'C', 'D2', 'D4', 'D7', 'D8', 'E', 'F', 'G', 'H', 'I', 'J', 'L', 'M', 'N', 'O', 'P', 'Q', 'R',
             'S', 'T',             'U', 'V', 'W', 'Y', 'Z', '0'] # The list of control area on which the assessment is performed.
...
class Branch:
    ...
    def coupleNodes(self, node1, node2):
        if self.nameFrom == node1:
            ...
            print(
                "Error while merging branch : " + self.nameBranch + " node:" + node1 + " was not found among " + self.nameFrom + " and " + self.nameTo)

    def __str__(self):
        return str(self.index) + "," + self.nameBranch + "," + self.nodeFrom.name + "," + self.nodeTo.name + "," + str(
            self.impedance) + "," + str(self.PATL) + "," + str(self.ring) + "," + str(self.tieLine)
...
class Node:
    ...
    def __str__(self, ):
        return str(self.index) + "," + self.name + "," + str(self.ring) + "," + str(self.connected) + "," + str(
            [elt.index for elt in self.branches])
...
@jit(
    'void(int32[:], float64[:], float64[:], float64[:,:], float64[:,:], float64[:,:], float64[:,:], int32[:,:], float64[:,:], int32[:], int32[:], int32[:], float64[:], float64[:], int32[:], float64[:,:])')
...
@jit(
    'void(int32[:,:], float64[:,:], int32[:,:], float64[:,:], float64[:,:], int32[:], int32[:], int32[:], int32[:], float64[:], float64[:], float64[:])')
def getResults(resultsT, resultsIF, resultsNT, resultsNIF, resultsNIFNN, finalResultsT, finalResultsNT, finalResultsI,
               finalResultsNI, finalResultsIF, finalResultsNIF, finalResultsNIFNN):
            ...
            if len(fileRead[i]) > 80:
                ...
                try:
                    ...
                    if generatorPower >= 0.0:
                        fileLog.write(
                            "     Generator " + nodeName + " has negative or zero maximum generation power" + '\n')
                    else:
                        generators.append(GenerationUnit(nodeName, -generatorPower))
                except:
                    fileLog.write(
                        "     Generator " + nodeName + " maximum permissible generation could not be read." + '\n')
            i += 1
    else:
...
def attachGenerators(setOfNodes, setOfElements):
    ...
    for elt in setOfElements:
        ...
        if len(attachedNode) == 1:
            ...
            fileLog.write("     Generator " + elt.name + " could not be attached to node " + elt.nodeName + " : " + str(
                len(attachedNode)) + " matches found." + '\n')
    generatorsToRemove = []
    for elt in setOfElements:
    ...
    for i in range(len(setOfElements)):
        setOfElements[i].index = i
    print(str(len(setOfElements)) + " generators are in operation in the system. " + str(
        len(generatorsToRemove)) + " generators are out of operation.")
...
def mergeEquivalents(setOfElements, countryCode):
    ...
    eltToMerge = [elt for elt in branches if
                  elt.impedance < 0 and elt.nameFrom[0:len(countryCode)] == countryCode and elt.nameTo[0:len(
    ...
    for eltEq in eltToMerge:
        for (nodeEq, nodeReal) in [(eltEq.nameFrom, eltEq.nameTo), (eltEq.nameTo, eltEq.nameFrom)]:
            if int(nodeEq[6:7]) < int(nodeReal[6:7]):
                ...
                if len(eltReals) == 0:
                    print(eltEq.nameBranch + " has negative impedance but there are " + str(
                        len(eltReals)) + " real transformers found.")
                else:
                    for elt in eltReals:
                        eltReal = elt[0]
    ...
    eltToMerge = [elt for elt in branches if
                  elt.impedance < 0 and elt.nameFrom[0:len(countryCode)] == countryCode and elt.nameTo[0:len(
                      countryCode)] == countryCode]
    print("Negative impedance transformers merged in control area " + countryCode + ": " + str(
        len(eltToMerge)) + " such elements remaining")
...
def mergeTieLines(setOfElements, setOfNodes):
    ...
    for nodeToMerge in [node for node in setOfNodes if node.isXNode()]:
        if len(nodeToMerge.branches) != 2:
            fileLog.write(
                "     X-node " + nodeToMerge.name + " could not be merged : incorrect number of branches connected, " + str(
                    len(nodeToMerge.branches)) + '\n')
            if len(nodeToMerge.branches) == 1:
                nodeToMerge.remove(setOfElements, setOfNodes)
                fileLog.write("     Node " + nodeToMerge.name + " and branch " + nodeToMerge.branches[
                    0].nameBranch + " removed." + '\n')
        else:
            ...
            if len(nodesA) != 1:
                fileLog.write(
                    "     Error while merging " + nodeToMerge.name + " : incorrect number of nodes for branch " + branchA.nameBranch + '\n')
                continue
            else:
                nodeA = nodesA[0]
            if len(nodesB) != 1:
                fileLog.write(
                    "     Error while merging " + nodeToMerge.name + " : incorrect number of nodes for branch " + branchB.nameBranch + '\n')
                continue
            else:
                nodeB = nodesB[0]
            ...
            if orderA != orderB:
                fileLog.write(
                    "     Error while merging " + nodeToMerge.name + " : order could not be determined for lines " + branchA.nameBranch + " and " + branchB.nameBranch + '\n')
                mergedOrder = "X"
            else:
                mergedOrder = orderA
            ...
            nodeToMerge.remove(setOfElements, setOfNodes)
            fileLog.write(
                "     X-node " + nodeToMerge.name + " merged : " + branchA.nameBranch + " and " + branchB.nameBranch + " merged in " + mergedBranch.nameBranch + '\n')
            fileLog.write("     Branch A impedance : " + str(branchA.impedance) + ", branch B impedance : " + str(
                branchB.impedance) + ", merged branch impedance : " + str(mergedBranch.impedance) + '\n')
...
def initializeRingAndConnection(setOfNodes, countryCode):
    ...
    mostConnectedNode = \
    [elt for elt in setOfNodes if elt.name[0:len(countryCode)] == countryCode and len(elt.branches) == maxConnection][0]
    ...
    # Listing nodes which are not connex to the assessed control area
    print("Among " + str(len(setOfNodes)) + " nodes, " + str(
        len([elt for elt in nodes if elt.connected == False])) + " nodes are not connected to " + countryCode)
...
def mainComponentRestriction(setOfNodes, setOfElements):
    ...
    print("System restricted to main connected component with " + str(len(connexNodes)) + " nodes and " + str(
        len(connexBranches)) + " elements")
...
def computeISF(setOfNodes, setOfElements):
    ...
    maxConnection = max([len(node.branches) for node in setOfNodes if
                         node.name[0:len(countryCode)] == countryCode and node.name[6:7] == "1"])
    slackNode = [node for node in setOfNodes if
                 len(node.branches) == maxConnection and node.name[0:len(countryCode)] == countryCode][0]
...
def determineIext(setOfElements, setT, inputLODF, normalisationMatrix):
    ...
    while len(setJ) > 0:
        ...
        for i in range(len(setJ)):
            ...
            setIext.append(eltI)
            fileI.write(eltI.nameBranch + "," + str(eltI.ring) + "," + str(eltI.PTDF) + "," + str(
                numpy.amax(LODF[:, i])) + "," + str(numpy.amax(LODFn[:, i])) + '\n')
        currentRing += 1
        setJ = [elt for elt in setOfElements if elt.ring == currentRing]
    fileI.close()
    print(
        "External contingencies determinated : " + str(len(setIext)) + " elements selected within maximum ring #" + str(
            currentRing))
    return setIext
...
def excludeRadialI(setJ):
    ...
    for eltI in setJ:
        if eltI.PTDF > 1 - epsilon:
            ...
            result.append(eltI)
    print("Radial elements which do not lead to disconnection of a generetor are excluded : " + str(
        len(result)) + "/" + str(len(setJ)) + " kept.")
    return result
...
def computeIF(inputLODF, normalizationMatrix):
    ...
    while len(setR) > 0:
        ...
        resultsT = numpy.zeros((sizeI, sizeR),
                               dtype=numpy.int32)  # 2D-Matrix of most influenced t element in N-i-r situation
        resultsNT = numpy.zeros((sizeI, sizeR),
                                dtype=numpy.int32)  # 2D-Matrix of most normalized influenced t element in N-i-r situation
        resultsIF = numpy.zeros(
            (sizeI, sizeR))  # 2D-Matrix of influence factor on the most influenced t element in N-i-r situation
        resultsNIF = numpy.zeros((sizeI,
                                  sizeR))  # 2D-Matrix of normalized influence factor on the most normalized influenced t element in N-i-r situation
        resultsNIFNN = numpy.zeros((sizeI,
                                    sizeR))  # 2D-Matrix of non-normalized influence factor on the most normalized influenced t element in N-i-r situation
    ...
    for r in range(len(setR)):
        ...
        for i in range(len(setI)):
            ...
            if IF > results[r][2]:
                ...
                results[r][4].append([setT[k].nameBranch for k in range(len(setT)) if
                                      abs(vectorLODF[k]) == IF and not setT[k] in results[r][4]])
            ...
            if IFn > results[r][5]:
                ...
                results[r][7].append([setT[k].nameBranch for k in range(len(setT)) if
                                      abs(vectorLODFn[k]) == IFn and not setT[k] in results[r][7]])
    return results
